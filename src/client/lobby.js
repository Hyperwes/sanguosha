...
const EXPANSIONS = ['wind', 'fire', 'wood', 'hills', 'ge1', 'ge2', 'ge3', 'ge4', 'ge5'];
...
export default class SanGuoShaLobby extends React.Component {
    ...
    maybeRenderCreateButton() {
        ...
        return <button
			onClick={() => {
				const newState = {};
				EXPANSIONS.forEach(expansion => newState[`expansion-${expansion}`] = false);
				this.setState(newState, () => 
					this.createMatch(SanGuoSha.maxPlayers, undefined)
    ...
    renderMatch = match => {
        ...
        if (gameover) {
            ...
            status = ['Expansions:', ...EXPANSIONS.map(expansion => <span key={expansion} className='expansion'>
                <input
                    type='checkbox'
                    value={this.state[`expansion-${expansion}`] || this.state['expansion-all']}
        ...
        if (matchInfo === undefined || matchInfo.matchID !== matchID) {
            ...
            if (playerNames[0] === name && playerNames.length >= SanGuoSha.minPlayers) {
				...
				buttons.push(
					 <button
						key="start with all expansions"
						onClick={() => {
							const newState = {};
							EXPANSIONS.forEach(expansion => newState[`expansion-${expansion}`] = true);
							this.setState(newState, 
											() => {
												this.createMatch(playerNames.length, matchID).then(this.refreshLobbyState);
											})
						}}
					>
						{'Start with all expansions'}
					</button>
				);
            }
            buttons.push(

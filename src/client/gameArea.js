...
export default class GameArea extends React.Component {
    ...
    render() {
        ...
        playerAreas.forEach((playerArea, i) => {
            ...
            this.addPlayerCard(playerArea, player, normalCards, nodes);
            if (player !== playerID) {
    ...
    addCharacterCard(playerArea, character, player, characterCards) {
        ...
        } else if (mode === SetModePanel.HEAVENLY_ESSENCE_MODE) {
            if (selectedIndex === undefined) {
                onClick = () => this.setState({ selectedIndex: player });
            } else {
                onClick = () => {
                    moves.switchEquipment(selectedIndex, player);
                    this.setState({ mode: SetModePanel.DEFAULT_MODE, selectedIndex: undefined });
                };
            }
        }
    ...
    addPlayerEquipment(playerArea, player, normalCards) {
        ...
        ['Weapon', 'Shield', '+1', '-1', 'Lightning', 'Capture', 'Starvation'].forEach((category, i) => {
            ...
            if (card) {
                ...
                } else if (mode === SetModePanel.CARD_ON_CHAR_MODE) {
					onClick = () => {
						(mode === SetModePanel.CARD_ON_CHAR_MODE ? moves.characterPlaceFromOtherPlayer : moves.steal)({
							playerID: player,
							category,
						});
						this.setState({ mode: SetModePanel.DEFAULT_MODE });
    ...
    addPlayerCard(playerArea, player, normalCards, nodes) {
        ...
        hand.forEach(card => {
            ...
            normalCards.push({
                ...
                sideways: true,
                opacity: 1,
                left: playerArea.x + scaledWidth * 0.5,
                top: playerArea.y + scaledHeight * 0.2,
                scale: CARD_RATIO/1.5,
                onClick,
            });
        });
        // Show the card count
        if (hand.length > 0) {
            nodes.push(<div
                key={`card-count-char-${player}`}
                className='game-label2'
                style={{
                    left: playerArea.x + scaledWidth * 0.3,
                    top: playerArea.y + scaledHeight * 0.2,
                    ...
                    fontSize: scaledWidth * CARD_RATIO * 0.4,
                }}
            >
                {hand.length}
            </div>);
    ...
    getDiscardCards(middleCardsFound) {
        ...
        for (let i = 0; i < discard.length && i <= MAX_DISCARDS_SHOWN; i++) {
            ...
            } else if (mode === SetModePanel.CARD_ON_CHAR_MODE && i < MAX_DISCARDS_SHOWN) {
                onClick = () => {
                    moves.characterPlaceFromDiscard(discard.length - 1 - i)
                    this.setState({ mode: SetModePanel.DEFAULT_MODE });
                };
			} else if (mode === SetModePanel.CARD_ON_DECK_MODE && i < MAX_DISCARDS_SHOWN) {
                onClick = () => {
    ...
    renderActionButton() {
        ...
        } else if (mode === SetModePanel.HEAVENLY_ESSENCE_MODE && selectedIndex !== undefined) {
            actionButton = {
                text: 'Select player 2',
                type: 'disabled',
            };

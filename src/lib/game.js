...
function characterPlaceFromDiscard(G, ctx, index) {
    const { cardsOnChar, discard} = G;
    const { playerID } = ctx;
    const [card] = discard.splice(index, 1);
    if (card === undefined) {
        return;
...
function characterPlaceFromOtherPlayer(G, ctx, target) {
    const { hands, equipment, cardsOnChar } = G;
	const { playerID } = ctx;
...
function dismantle(G, ctx, target) {
    const { hands, equipment, cardsOnChar } = G;
    if (target.cardOnCharBool) {
        const [card] = cardsOnChar[target.playerID].splice(target.index, 1);
        discard(G, ctx, card);
    }
...
function switchEquipment(G, _ctx, player1, player2) {
	...
	['Weapon', 'Shield', '+1', '-1'].forEach((category, i) => {
            ...
            if (card && card2) {
                equipment[player1][category] = undefined;
				equipment[player2][category] = undefined;
				equipment[player2][category] = card;
				equipment[player1][category] = card2;
			} else if (card) {
				equipment[player1][category] = undefined;
				equipment[player2][category] = card;
			} else if (card2) {
...
function awaken(G, ctx) {
    const { healths } = G;
    const { playerID } = ctx;
    healths[playerID].max--;
    if (healths[playerID].max < 0) {
        healths[playerID].max = 6;
    }
    if (healths[playerID].current > healths[playerID].max) {
        healths[playerID].current--;
    }
...
function reverseAwaken(G, ctx) {
    const { healths } = G;
    const { playerID } = ctx;
    healths[playerID].max++;
    if (healths[playerID].max > 6) {
